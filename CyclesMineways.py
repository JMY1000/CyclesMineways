...
LIGHT_TRANSPARENT_BLOCK = ["Beacon", "Endframe", "End_Rod", "Fire", "Powered_Rail_(active)", "Redstone_Comparator_(active)", "Redstone_Torch_(active)", "Repeater_(active)", "Torch"]
...
def light_emitting_shader(material):
    nodes, node_tree = setup_node_tree(material)
    # Create the output node
    output_node = nodes.new('ShaderNodeOutputMaterial')
...
def transparent_light_emitting_shader(material):
    nodes, node_tree = setup_node_tree(material)
    # Create the output node
    output_node = nodes.new('ShaderNodeOutputMaterial')
    output_node.location = (600, 300)
...
def main():
    ...
    for material in materials:
        if (material.active_texture and len(material.active_texture.name)>= 2 and material.active_texture.name[0:2] == "Kd"):
            ...
            elif any(material == bpy.data.materials.get(lightBlock+material_suffix) for lightBlock in LIGHT_BLOCKS):
                print(material.name+" is light block.")
                light_emitting_shader(material)
            # if the material is a light emmitting transparent block use a special shader
            elif any(material == bpy.data.materials.get(LIGHT_TRANSPARENT_BLOCK+material_suffix) for LIGHT_TRANSPARENT_BLOCK in LIGHT_TRANSPARENT_BLOCK):
                print(material.name+" is transparent light block.")
                transparent_light_emitting_shader(material)
